    ...
    <div class="form-container">
        <div class="form-header">
            <div class="header-title">
                <h1>Edit Jadwal Penerbangan</h1>
                <p>Perbarui informasi rute atau waktu untuk jadwal ID: #{{ $schedule->id }}</p>
            </div>
        ...
        <div class="form-card">
            <form action="{{ route('schedule.update', $schedule->id) }}" method="POST">
                    ...
                    <div class="form-section">
                            ...
                            <select name="plane_id" class="form-input" required>
                                @foreach ($planes as $plane)
                                    <option value="{{ $plane->id }}"
                                        {{ $schedule->plane_id == $plane->id ? 'selected' : '' }}>
                        ...
                        <label class="label-group">
                            ...
                            <div class="input-with-icon">
                                ...
                                <input type="number" name="price" value="{{ $schedule->price }}" class="form-input"
                                    required>
                            </div>
                        </label>
                                ...
                                <select name="origin_id" class="form-input" required>
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ $schedule->origin_id == $city->id ? 'selected' : '' }}>
                                            {{ $city->name }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div class="form-footer">
                    <a href="{{ route('schedule.index') }}" class="btn-reset"
